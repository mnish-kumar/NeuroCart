...
async function createOrder(req, res) {
  ...
  try {
    const cartResponse = await axios.get(`neurocart-ALB-441288313.ap-south-1.elb.amazonaws.com/api/cart/`, {
  ...
  const products = await Promise.all(
    cartResponseData.items.map(async (item) => {
      return (
        await axios.get(
          `neurocart-ALB-441288313.ap-south-1.elb.amazonaws.com/api/products/${item.productId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
            timeout: CART_SERVICE_TIMEOUT_MS,
          },
